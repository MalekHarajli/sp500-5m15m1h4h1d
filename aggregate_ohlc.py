...
def get_checkpoint(key: str) -> datetime:
    """Fetch last processed timestamp from aggregation_state."""
...
# ============================================================
# 2. FETCH RAW MINUTE DATA (CORRECT COLUMN: ts)
# ============================================================

def fetch_minute_chunk(since_timestamp: datetime):
    """Fetch up to CHUNK_SIZE minute_ohlc rows newer than ts."""
...
def aggregate_rows(rows, timeframe_minutes):
    ...
    for r in rows:
        ...
        bkey = (r["symbol"], bucket.isoformat())
...
def process_5m():
    ...
    last_processed_ts = datetime.fromisoformat(
        minute_rows[-1]["ts"].replace("Z", "+00:00")
    )
...
def process_1d():
    ...
    print(f"Inserted {len(agg)} daily candles.")
    return True
